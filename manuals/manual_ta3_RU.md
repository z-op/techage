...
Система хранения указывает свою емкость в кудах, то есть в кю в день. Например, система хранения с емкостью 100 kud обеспечивает 100 ku в течение одного игрового дня или 10 ku в течение 10 игровых дней.
...
Для нанесения штукатурки необходимо кликнуть на кабель шпателем. Материал, которым будет заштукатурен кабель, должен находиться слева в быстрых слотах игрока.
...
### TA Power Pole
Используется для строительства столбов электропередач. Защищен от разрушения оголовком столба и может быть сломан только владельцем.
...
Генератор малой мощности работает на бензине и может использоваться для небольших потребителей мощностью до 12 ku. При полной нагрузке бензин сгорает в течение 150 с. Соответственно дольше при частичной нагрузке (50% нагрузки = трата вдвое дольше).

Электрогенератор вмещает только 50 единиц бензина. Поэтому ваш выбор это установить дополнительный бак и насос.
...
Аккумуляторный блок (Далее accu) используется для накопления избыточной энергии и автоматически подает энергию в случае отключения питания (при наличии).
...
Здесь всегда отображаются значения для всей сети. Накопленная нагрузка отображается в "kud-ах" или "ku-днях" (аналогично кВт-ч). 5 куд соответствует, например, 5 ku за игровой день (20 минут) или 1 ku за 5 игровых дней.

Блок accu имеет 3,33 kud
...
Масляная горелка вмещает только 50 единиц топлива. Поэтому... угадайте! Нужно установить дополнительный бак и насос.
...
Управление клапаном также может осуществляться с помощью команд включения/выключения (см. TA3 Терминал).
...
Для этого используются исследователь нефти TA3, буровая установка TA3 и насосный домкрат TA3.

[techage_ta3|image].


### TA3 исследователь нефти

Вы можете искать нефть с помощью исследователя нефти. Для этого поставьте блок на пол/грунт/землю и щелкните правой кнопкой мыши, чтобы начать поиск. Нефтеискатель можно использовать как над землей, так и под землей на любой глубине.
...
Исследователь нефти всегда может быть использован для поиска нефти.
...
Буровая коробка имеет 4 стороны, в IN подается бурильная труба через толкатель, а в OUT вылетает буровой материал. Через одну из двух свободных сторон на буровую коробку должно подаваться питание.

Буровая коробка бурит до нефтяного месторождения (1 метр за 16 с) и требует 16 ku электроэнергии.
...
Теперь на место бурового ящика необходимо установить масляный насос (pump-jack). Нефтяной насос также требует электричества (16 ku) и подает одну единицу нефти каждые 8 с. Нефть должна собираться в резервуар. Для этого нужно соединить масляный насос с резервуаром с помощью желтых труб.
...
Логический блок TA3 может быть запрограммирован таким образом, что одна или несколько входных команд будут связаны с одной выходной командой и отправлены. Таким образом, этот блок может заменить различные логические элементы, такие как AND (И), OR (ИЛИ), NOT (НЕ), XOR (НЕ ИЛИ) и т. д.
...
```
<вывод> = on/off, if <выражение-ввод> true
```
